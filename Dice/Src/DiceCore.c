...
//        ob.WRPSector = OB_WRP_Pages0to31 | OB_WRP_Pages32to63 | OB_WRP_Pages64to95 | OB_WRP_Pages96to127 | OB_WRP_Pages128to159 | OB_WRP_Pages160to191 | OB_WRP_Pages192to223 | OB_WRP_Pages224to255;
    ...
    {
        EPRINTF("WARNING: Non-Permanent lockdown detected (RDPLevel = OB_RDP_LEVEL_1).\r\n");
    }
...
void DiceGenerateDFUString(void)
{
    ...
    for(uint32_t n = DICEBOOTLOADERSIZE; n < DICEFLASHSIZE; n += MIN(99 * 2048, DICEFLASHSIZE - n))
    {
        cursor += sprintf(&dfuStr[cursor],
                "/0x%08lx/%02lu*2Kf",
                (DICEFLASHSTART + n),
                MIN(99, (DICEFLASHSIZE - n) / 2048));
    }

    if((!DICERAMAREA->info.properties.noClear) && (!__HAL_FIREWALL_IS_ENABLED()))
    {
        // Read only the used memory, but allow to write the entire space back
        ...
        uint32_t writePages = (DICEDATASTART + DICEDATASIZE - writeStart) /64;
        cursor += sprintf(&dfuStr[cursor],
...
static DICE_STATUS DiceWriteData(uint32_t* destination, uint32_t* dataWords, uint32_t numDataWords)
{
    HAL_StatusTypeDef status;
    FLASH_EraseInitTypeDef eraseInfo = {FLASH_TYPEERASE_PAGES, FLASH_BANK_1, 255, 1};
    uint32_t pageError = 0;

    // Copy the page top RAM
    uint8_t* pageCopy = malloc(DICEDATASIZE);
    if(pageCopy == NULL) return DICE_FAILURE;
    memcpy(pageCopy, DICEDATASTART, DICEDATASIZE);

    // Patch in the new data
    uint32_t* newDestination = (uint32_t*)(((uint32_t)destination - DICEDATASTART) + (uint32_t)pageCopy);
    for(uint32_t n = 0; n < numDataWords; n++)
    {
        newDestination[n] = dataWords[n];
    }

    // Flash the entire page back
    status = HAL_FLASH_Unlock();
    if(status != HAL_OK) return DICE_FAILURE;
    status = HAL_FLASHEx_Erase(&eraseInfo, &pageError);
    if((status != HAL_OK) || (pageError != 0xffffffff)) return DICE_FAILURE;
    uint32_t target;
    uint64_t data;
    for(uint32_t n = 0; n < ((DICEDATASIZE / sizeof(uint64_t))); n++)
    {
        target = (uint32_t)&(((uint64_t*)DICEDATASTART)[n]);
        data = ((uint64_t*)pageCopy)[n];
        if(data == 0xFFFFFFFFFFFFFFFF)
        {
            continue;
        }
        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, target, data);
        if(status != HAL_OK)
        {
            return DICE_FAILURE;
        }
...
//        staging->info.certBagLen = DERtoPEM(&cerCtx, 0, staging->info.certBag, staging->info.certBagLen);
        ...
        if((retVal = DiceWriteData((uint32_t*)DICEDATASTART, stagingBuffer, (staging->info.dontTouchSize / sizeof(uint32_t)))) != DICE_SUCCESS)
        {
            goto Cleanup;
        }
        free(cerBuffer);
    ...
    // Increment the BootCounter
    {
        ...
        if((retVal = DiceWriteData(&DICEFUSEAREA->info.bootCounter, &BootCtr, 1)) != DICE_SUCCESS)
        {
    ...
    if(!DiceNullCheck(&DICERAMAREA->info.authorityPub, sizeof(ecc_publickey)))
    {
        ...
        if(DICEAPPHDR->s.sign.issueDate > DICERAMAREA->info.rollBackProtection)
        {
            if((retVal = DiceWriteData(&DICEFUSEAREA->info.rollBackProtection, &DICEAPPHDR->s.sign.issueDate, 1)) != DICE_SUCCESS)
            {
...
DICE_STATUS DiceSecure(void)
{
    ...
    fw_init.NonVDataSegmentStartAddress = DICEDATASTART;
    fw_init.NonVDataSegmentLength = DICEDATASIZE;
